...
CREATE OR REPLACE PACKAGE PKG_BODEGA AS
    PROCEDURE LEER_TODAS (P_CURSOR OUT SYS_REFCURSOR);
END PKG_BODEGA;
/

CREATE OR REPLACE PACKAGE BODY PKG_BODEGA AS
    PROCEDURE LEER_TODAS (P_CURSOR OUT SYS_REFCURSOR) AS
    BEGIN
        OPEN P_CURSOR FOR SELECT BODEGA_ID, NOMBRE FROM BODEGA WHERE ACTIVO = 1;
    END;
END PKG_BODEGA;
/